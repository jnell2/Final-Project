...
def add_rows(df_all, df_games, home_team, away_team, date):
    ...
    df_final5 = dc.cumulative_stats(df_all, df_games, 2)
...
if __name__ == '__main__':
    ...
    df_final2_LS = pd.read_csv('data/final2LS.csv')
    df_final2_LS.drop(['Unnamed: 0', 'home_giveaways', 'away_giveaways'], axis = 1, inplace = True)

    # gets data that we want to predict on, if don't want to add new rows (past games)
    df_final2 = pd.read_csv('data/final2.csv')
    df_final2.drop(['Unnamed: 0', 'home_giveaways', 'away_giveaways'], axis = 1, inplace = True)

    # if you want to know new games, this appends rows to past games to make 1 big df
    df_all, df_games = get_data()
    df_games, df_final2_new = add_rows(df_all, df_games, 'BUF', 'TBL', '2016-11-17')
    ...
    # last season
    df_finalLS = df_final2_LS[['home_team', 'away_team', 'date', 'home_team_win']]
